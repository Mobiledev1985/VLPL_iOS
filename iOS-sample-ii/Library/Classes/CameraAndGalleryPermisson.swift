...
class CameraAndGalleryPermisson: NSObject{
    var cameraType : CameraStyle = .front
...
//            vc.showAlertWithOkAndCancelHandler(string: Localized(string: "ALERT_CAMERA_PERMISSION"), strOk: Localized(string: "SETTING"), strCancel: Localized(string: "CANCEL"), handler: { (isSettings) in
//                if isSettings{
...
//            })
    ...
    func openPhotoLibrary(_ vc : UIViewController,isEdit : Bool,_ imageComplition : @escaping imageComplition){
        self.checkPhotoLibraryPermission(authorizedRequested: {
            ...
            //deniedRequested
//            vc.showAlertWithOkAndCancelHandler(string: Localized(string: "ALERT_PHOTO_GALLARY_PERMISSION"), strOk: Localized(string: "SETTING"), strCancel: Localized(string: "CANCEL"), handler: { (isSettings) in
//                if isSettings{
//                    UIApplication.shared.open(URL(string:UIApplication.openSettingsURLString)!, options: [:], completionHandler: { (_ ) in
//
//                    })
...
extension CameraAndGalleryPermisson : UIImagePickerControllerDelegate,UINavigationControllerDelegate{
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        var strImageName = ""
        if (info[.originalImage] as? NSURL) != nil {
            let imageUrl          = info[.originalImage] as! NSURL
...
extension UIViewController {
    ...
    func showAlertWithOkHandler(string: String,strOk:String,handler: @escaping ()->Void)
    {
        
        let alert : UIAlertController = UIAlertController(title: "", message: string, preferredStyle: .alert)
        
        let alertOkayAction = UIAlertAction(title: strOk, style: .default) { (alertAction) in
            handler()
