...
//    }
    ...
    /// Set fonts with specified name and size
    ///
    /// - Parameters:
    ///   - fontname: Fonts are from OpenSans Category Used by Gobe.
    ///   - size: default font size is 15.0
    /// - Returns: UIFont with specified fontname and/or size
    func setFont(fontname : appFonts, with size : CGFloat = 15.0) -> UIFont {
...
extension UIViewController {
    ...
    class func instantiateFromStoryboard(_ name: enStoryboardName) -> Self
    {
    ...
    func showAlertView(_ baseModel : BaseModel){
        if baseModel.responseMessage.string.count == 0 {
            return;
        }
        let alertController = UIAlertController(title: APPNAME, message: baseModel.responseMessage, preferredStyle: UIAlertController.Style.alert)
